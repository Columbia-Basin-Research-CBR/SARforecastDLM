...
run_model_and_forecast <- function(data, inputQ, inputB, inputR, inputU) {
  ...
  ## forecast
  forecast_ytt1<-MARSS::forecast(dlm, h= 1, newdata = list(z = index_z_test, y = train.data[nrow(train.data),2]), type = "ytt1", interval = "confidence", fun.kf = "MARSSkfss") 

  
    # Determine the year vector based on the condition
  if (unique(data$sar.method) == "DART" && unique(data$reach) == "BON_BOA") {
  ...
  forecast_df <- forecast_ytt1$pred %>% 
                    ...
                    select(year, value, y, estimate, lo_95, hi_95,sar.method,index, rear_type, pass_type, dataset, convergence)
...
```{r fct_MARSS_model_example}
params<-paramlist %>% filter(sar.method == "CJS", index == "CUTI", reach == "LGR_LGA") 

# #example
mod<-run_model_and_forecast(
  data = sar_raw_data %>% filter(sar.method == "CJS", index == "CUTI", reach == "LGR_LGA"),
...
for(i in seq_len(nrow(combinations))) {
  ...
  combination_name <- paste(combinations$sar.method[i], combinations$index[i], combinations$reach[i], sep = "_")
  
  # Store the forecast in the list with the created name
  forecasts[[combination_name]] <- forecast
}
...
```{r run_model_and_forecast_loop_using_ifelse}
load(here::here("data", "sar_raw_data.rda"))
