...
```{r fct_MARSS_model}
# Define the function
run_model_and_forecast <- function(data, Qvarcov) {
  ...
  Q <- Qvarcov                       ## if DART, set to unconstrained, otherwise diagonal and unequal
  ...
  plot(dlm, silent = TRUE, plot.type = c("fitted.ytT", "xtT"))
...
for(i in seq_len(nrow(combinations))) {
  ...
  if(combinations$sar.method[i] == "DART"){
    Qvarcov_adjusted <- "unconstrained"
  } else {
    Qvarcov_adjusted <- "diagonal and unequal"
  }
  # Run the model and get the forecast
  forecast <- run_model_and_forecast(data_subset, Qvarcov = Qvarcov_adjusted)
