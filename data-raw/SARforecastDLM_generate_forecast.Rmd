...
#### Smolt-to-adult (SAR) survival via CBR
...
#dataset to pred (includes CUI for 2022 and 2023 with no SAR)
sar.pred<-W_SAR %>% 
  mutate(year = as.numeric(year)) %>% 
  select(year, meanSAR) %>% 
  mutate( logit.s = car::logit(meanSAR)) %>% 
...
#### fit DLM (with NA for SAR 2022/2023)
...
```{r dlm-CUISAR--plotdlm-3, eval=TRUE, echo=FALSE, fig=TRUE, fig.height=4, fig.width=6, fig.cap='(ref:plotdlm-1)'}
ylabs <- c(expression(alpha[t]), expression(beta[t]))
colr <- c("darkgreen","blue")
par(mfrow = c(m, 1), mar = c(4, 4, 0.1, 0), oma = c(0, 0, 2, 0.5))
for(i in 1:m) {
...
```{r dlm-CUISAR-plotdata, eval=TRUE, echo=FALSE }
...
# Use augmented to save only untransformed for shiny plot
plot_data <- data.frame(
  years = years, #years
  sar = invLogit(t(dat)),  #observed sar data
  CUI_z = Z[1,2,] #predictor variable scaled 
...
Code below is used for SAR plot featured on shinyAPP
