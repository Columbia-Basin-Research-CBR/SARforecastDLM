...
fct_year<-function(data, years_selected = 2000, index_selected = "CUI"){
...
forecast_df_raw<-forecast_df_pred%>% 
  ...
  mutate(year = 1964:(years_selected+3),
         index = "CUI", 
...
df_raw<-fct_year(data = full_data)


base_data<-plot_data_1 %>% 
  filter(sar.method == "Scheuerell and Williams (2005)", 
         index == "CUI") %>% 
  rename("value" = upwelling_index) %>% 
  mutate(dataset = "base_data")
  

fct_forecast_compare_plot(data = df_raw, data_1 = base_data)
